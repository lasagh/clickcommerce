...
<fieldset class="group-select" id="supercheckout-fieldset">
	...
	{/if}
		...
		<div class="supercheckout-checkout-content"></div>
                            ...
                            {foreach from=$settings['shipping_address'] key='p_address_key' item='p_address_field'}
                                ...
                                {if $settings['shipping_address'][$p_address_key][$user_type]['display'] eq 1 || (isset($settings['shipping_address'][$p_address_key]['conditional']) && $settings['shipping_address'][$p_address_key]['conditional'] eq 1)}
                                    ...
                                    {if $p_address_key eq 'address1' || $p_address_key eq 'address2' || 
                                         $p_address_key eq 'id_state' || $p_address_key eq 'number' ||
                                         $p_address_key eq 'other' || $p_address_key eq 'city'}
                                        {$extraFieldClass = 'address-field'}
                                    {else}
                                        {$extraFieldClass = ''}
                                    {/if}
				    ...
				    {else}
                                    <tr class="sort_data {$extraFieldClass}" data-percentage="{$settings['shipping_address'][$p_address_key]['sort_order']|intval}" style="{$display_row|escape:'htmlall':'UTF-8'}" >
                                       {/if}
				       <td>{l s={$settings['shipping_address'][$p_address_key]['title']|escape:'htmlall':'UTF-8'} mod='supercheckout'}:<span style="display:{if $settings['shipping_address'][$p_address_key][$user_type]['require'] eq 1}inline{else}none{/if};" class="supercheckout-required">*</span>
                                            {if $p_address_key eq 'id_country' || $p_address_key eq 'id_state'}
                                                <select id="{$p_address_key|escape:'htmlall':'UTF-8'}" name="shipping_address[{$p_address_key|escape:'htmlall':'UTF-8'}]" class="supercheckout-large-field">
                                            ...
                                            {else if $p_address_key eq 'postcode'}
                                                ...
                                                <input type="hidden" id="{$p_address_key|escape:'htmlall':'UTF-8'}" name="shipping_address[{$p_address_key|escape:'htmlall':'UTF-8'}]" />
                                            {else}
                                                <input type="text" id="{$p_address_key|escape:'htmlall':'UTF-8'}" name="shipping_address[{$p_address_key|escape:'htmlall':'UTF-8'}]" value="" class="supercheckout-large-field" />
                                            {/if}
